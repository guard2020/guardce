    ...
    <nav class="flex items-center justify-between text-black bg-navbar shadow-xs h-16">
        <div class="flex items-center flex-no-shrink">
            <a href="{{ url('/') }}" class="flex items-center flex-no-shrink text-black mx-4">
                <img class="guard-img-margin mx-3" style="height: 1.25rem;" src="{!! asset('images/guard/icons/main_icon_white.png') !!}" alt="">

                <p class="inline-block font-semibold ml-5 mr-1 text-grey-dark">
                    {{ config('app.name') }}
                </p>
            </a>
                ...
                <larecipe-dropdown>
                    ...
                    <template slot="list">
                      {{--  <form action="/logout" method="POST">
                        ...
                        <a href="{!! route('logout') !!}" class="dropdown-item py-2 px-4 text-white bg-danger inline-flex"><i class="fa fa-power-off mr-2"></i> Logout</a>
                    </template>
                </larecipe-dropdown>
                {{-- /account --}}
            @endauth
