    ...
    @if(!empty($selectedInstanceArr))
        ...
        <div id="content" class="tab-content" role="tablist">
            <div id="pane-A" class="tab-pane fade show active " role="tabpanel" aria-labelledby="tab-A">
                @include('security-pipeline.partials.agent-parameter-panel')
            </div>

            <div id="pane-B" class="tab-pane fade" role="tabpanel" aria-labelledby="tab-B">
                @include('security-pipeline.partials.agent-resource-panel')
            </div>
        </div>
    @else
        <div class="col-12">
            <div class="alert alert-info border-0 alert-dismissible">
                <span class="font-weight-semibold">No agent instance selected!</span> Please select at least one instance
            </div>
        </div>
    @endif
</div>

@include('security-pipeline.partials.js.agent-parameters-scripts')
