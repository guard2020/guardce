#!/bin/bash
#### Usage start_config.sh internal_ip_address cb-manager-port lcp-port es-port kibana-port

if [ "$#" -ne 5 ]; then
        echo "Usage: ./start_config.sh internal_ip_address cb-manager-port lcp-port es-port kibana-port"
        exit 1
fi
